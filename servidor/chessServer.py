...
def juegaServidor():
    result = engine.play(board, chess.engine.Limit(float(TiempoDeReflexion)))
...
cliente.recv(1)  # Recibe una espera del cliente.
# ...Se asegura de enviar 4 bytes en el tiempo al cliente.
TiempoDeReflexion=str(TiempoDeReflexion)
TiempoDeReflexion=bytes(TiempoDeReflexion.zfill(4), "utf-8")                                                                                      
cliente.send(TiempoDeReflexion)  # Envía    el  tiempo para pensar al cliente.
